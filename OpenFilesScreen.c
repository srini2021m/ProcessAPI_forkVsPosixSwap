...
static OpenFiles_ProcessData* OpenFilesScreen_getProcessData(pid_t pid) {
   ...
   posix_spawn_file_actions_addclose(&actions, fdpair[0]);
   posix_spawn_file_actions_addclose(&actions, fdpair[1]);

   pid_t child;
   const char* args[] = {"lsof", "-P", "-o", "-p", NULL, "-F", NULL}; // NULL-terminated array of arguments
   char pid_str[12];
   snprintf(pid_str, sizeof(pid_str), "%d", pid);

   for (int i = 0; args[i] != NULL; i++){
      if (posix_spawn(&child, args[i], &actions, NULL, (char* const*)(args +i), NULL) != 0) {
         close(fdpair[1]);
         close(fdpair[0]);
         pdata->error = 1;
         return pdata;
